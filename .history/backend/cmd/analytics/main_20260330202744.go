...
func (w *AnalyticsWorker) handleFactorAnalysis(ctx context.Context, payload json.RawMessage) (interface{}, error) {
	...
	// 将Holdings转换为StockFactorData
	stocks := make([]analytics.StockFactorData, 0, len(holdings.Holdings))
	for _, h := range holdings.Holdings {
		stocks = append(stocks, analytics.StockFactorData{
			Symbol: h.Symbol,
			Name:   h.Name,
			Weight: h.Weight,
		})
	...
	// 计算因子暴露
	result := analytics.CalculateFactorExposure(req.Symbol, stocks, nil)
...
func (w *AnalyticsWorker) getHoldingsFromDB(ctx context.Context, symbol string) (analytics.ETFHoldings, error) {
	// 这里应该查询实际的数据库
	// 简化示例
	return analytics.ETFHoldings{Symbol: symbol, Holdings: []analytics.Holding{}}, nil
}
