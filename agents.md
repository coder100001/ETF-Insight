...
- ✅ **回测引擎**: 事件驱动回测、滑点模拟、交易成本建模
  - ✅ **v2.11.1 修复 (2026-05-13)**:
    - P0 年化收益率计算错误：使用正确的复利公式 `(final/initial)^(1/years) - 1`
    - P0 胜率统计 Bug：在 `Fill` 中记录 `RealizedPnL`，不依赖持仓删除
    - P0 策略接口空引擎：正确创建并配置临时回测引擎实例
    - 性能优化：临时引擎创建移到循环外，内存分配 O(n) → O(1)
    - 测试覆盖：新增 5 个测试用例覆盖所有修复场景
...
*本文档最后更新: 2026-05-06 (v2.10 质量优先演进版)*
*强制上下文绑定版本: v2.0*
